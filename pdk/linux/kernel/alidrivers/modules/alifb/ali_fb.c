...
extern  struct alifb_info * get_alifb_info();
static void set_gma_scale_info(osd_scale_param *uScaleParam)
{
	struct alifbio_gma_scale_info_pars pars;
    	struct vpo_io_get_info vpo_info;
	osd_scale_param *pscale_para;
				
	pscale_para = uScaleParam;
/*	priv->h_div = pscale_para->h_div;
	priv->h_mul = pscale_para->h_mul;
	priv->v_div = pscale_para->v_div;
	priv->v_mul = pscale_para->v_mul;
	printk("scale param h_div %d h_mul %d v_div %d v_mul %d\n"
	, priv->h_div, priv->h_mul, priv->v_div, priv->v_mul);*/

	memset(&vpo_info, 0, sizeof(vpo_info));
	vpo_ioctl((struct vpo_device *)hld_dev_get_by_id(HLD_DEV_TYPE_DIS, 0), VPO_IO_GET_INFO, (UINT32)(&vpo_info));
	
	pars.scale_mode =GMA_RSZ_DIRECT_RESIZE;// (priv->scale_mode == OSD_SCALE_DUPLICATE) ? GMA_RSZ_DIRECT_RESIZE: GMA_RSZ_ALPHA_COLOR;
					
	pars.h_src = pscale_para->h_div;
	pars.h_dst = pscale_para->h_mul;
	pars.v_src = pscale_para->v_div;
	pars.v_dst = pscale_para->v_mul;
	pars.tv_sys = vpo_info.tvsys;
	pars.uScaleCmd = GE_SET_SCALE_PARAM;
...
//	struct alifbio_gma_scale_info_pars scale_pars;
	...
	int i = 0;
	struct alifb_info * info=get_alifb_info();
...
//	copy_from_user((void *)&scale_pars, (void *)arg, sizeof(scale_pars));
	for(i = 0;i < VIDEO_MAX_GMA_REGION_NUM;i++){
		...
		if(gma_info->valid){
            ...
            gma_info->u_scale_cmd = pars.uScaleCmd;
            gma_info->uScaleParam = pars.uScaleParam;
            gma_info->tv_sys = pars.tv_sys;
            if(gma_info->u_scale_cmd == GE_SET_SCALE_PARAM) {
				gma_info->h_dst = pars.h_dst;
				gma_info->h_src = pars.h_src;
				gma_info->v_dst = pars.v_dst;
				gma_info->v_src = pars.v_src;
            }              
            #else
			gma_info->scale_mode = pars.scale_mode;
			gma_info->h_dst = pars.h_dst;
			gma_info->h_src = pars.h_src;
			gma_info->v_dst = pars.v_dst;
			gma_info->v_src = pars.v_src;
...
void OSD_SetRegionPos(UINT8 uRegionId,struct OSDRect* rect)
{
	...
	{		
		RET_CODE ret = RET_FAILURE;		
			...
			if(gma_info->valid){
				...
				printk("new gma region %d pos x %d y %d\n",
...
extern char usr_osd_scale;
extern char usr_ignore_alpha;
void osd_config()
{
	int dst_x, dst_y, dst_w, dst_h;
	int src_ratio,i;
	int src_posx,src_posy,pos_dx,pos_dy;
	int src_scalex,src_scaley,scale_dx,scale_dy;
	osd_scale_param scale_param;
	struct OSDRect region;
	osd_scale_param pars;
	UINT32 t_mode;
	int ret;
	...
	int osd_width=1280;
	int osd_height=720;
	printk("osd_config !!!!!!usr_osd_scale=%d\n",usr_osd_scale);
	//printk("osd_config  osd_ratio=%d \n",boardinfo->avinfo.osd_ratio);
	int ratio=100;
	if(usr_osd_scale>30&&usr_osd_scale<150)
		ratio=usr_osd_scale;
	else
		ratio=90;
	//simple_strtol(boardinfo->avinfo.osd_ratio,NULL,0);
	scale_param.h_div = osd_width;//1280;
	scale_param.v_div = osd_height;//720;

	vpo_ioctl(g_vpo_dev, VPO_IO_GET_OUT_MODE, (UINT32)(&t_mode));
	printk("setScaleRatio :%d********\n",t_mode);
	switch(t_mode) {
		case PAL:
		case LINE_576P_50_VESA:
			scale_param.h_mul = 720;
			scale_param.v_mul = 576;
		break;
		
		case NTSC:
			scale_param.h_mul = 720;
			scale_param.v_mul = 480;
		break;
		...
		case LINE_1080P_60_VESA:
			scale_param.h_mul = 1920;
			scale_param.v_mul = 1080;
		break;
		case LINE_4096X2160_24:
	        scale_param.h_mul = 4096;
	        scale_param.v_mul = 2160;
	        break;
	   case LINE_3840X2160_24:
	   case LINE_3840X2160_25:
	   case LINE_3840X2160_30:
	        scale_param.h_mul = 3840;
	        scale_param.v_mul = 2160;
	        break;
	}
	//ret=OSDDrv_ShowOnOff((HANDLE)handle,0);
	// printk(" OSDDrv_ShowOnOff:%d\n",ret);
	//change the video scale
	struct Rect rect_src = {0, 0, 720, 2880};
	struct Rect rest_dst;
	
	rest_dst.uStartX = (rect_src.uWidth * (100 - ratio)) / 200 + 2;
	rest_dst.uStartY = (rect_src.uHeight * (100 - ratio)) / 200 + 10;
    	rest_dst.uWidth = rect_src.uWidth * ratio / 100  - 4 ;
   	 rest_dst.uHeight = rect_src.uHeight * ratio / 100 - 20;
	
	vpo_zoom(g_vpo_dev, &rect_src, &rest_dst);

	dst_x = (scale_param.h_mul * (100 - ratio)) / 200;
	dst_y = (scale_param.v_mul * (100 - ratio)) / 200;	
	dst_w = scale_param.h_mul * ratio / 100;
	dst_h = scale_param.v_mul * ratio / 100;

	region.uWidth = dst_w;
	region.uHeight = dst_h;
	pars.h_div = osd_width;//1280;
	pars.v_div = osd_height;//720;
	if (src_ratio!=ratio){
	    src_posx= (scale_param.h_mul * (100 -src_ratio)) / 200;
	    src_posy = (scale_param.v_mul * (100 -src_ratio)) / 200;
	    pos_dx= (scale_param.h_mul * (ratio -src_ratio)) /(200*count_ratio);
	    pos_dy =(scale_param.v_mul * (ratio -src_ratio)) /(200*count_ratio);
	    
	    src_scalex= scale_param.h_mul * src_ratio / 100;
	    src_scaley = scale_param.v_mul * src_ratio / 100;
	    scale_dx= scale_param.h_mul *(ratio -src_ratio) / (100*count_ratio);
	    scale_dy= scale_param.v_mul * (ratio -src_ratio) / (100*count_ratio);


	    for(i=1;i<=count_ratio;i++){
	        pars.h_mul = src_scalex+scale_dx*i;
	        pars.v_mul = ( pars.h_mul -src_scalex)*(dst_h- src_scaley)/(dst_w-src_scalex)+src_scaley;
	        set_gma_scale_info((UINT32)&pars);
	        
	        region.uLeft = (scale_param.h_mul-pars.h_mul)/2;
	        region.uTop =( region.uLeft -src_posx)*( dst_y-src_posy)/(dst_x-src_posx)+src_posy;
	        OSD_SetRegionPos(0, &region);
	...
	pars.h_mul = dst_w;
	pars.v_mul = dst_h;
	printk("scale_param : h_div %d h_mul %d v_div%d v_mul %d\n",scale_param.h_div 
		, scale_param.h_mul, scale_param.v_div, scale_param.v_mul );

	//OSDDrv_Scale((HANDLE)handle, OSD_SCALE_WITH_PARAM, (UINT32)&pars);
	set_gma_scale_info((UINT32)&pars);
	...
	printk("scale_param.h_mul : scale_param.h_mul %d scale_param.v_mul %d ratio  %d \n",scale_param.h_mul 
		, scale_param.v_mul, ratio );
	printk("dst_x : x %d y %d w %d h %d\n",dst_x, dst_y, dst_w, dst_h );
}
#endif

#ifdef CONFIG_RPC_HLD_GMA
static struct alifb_info *m_fb_info = NULL;
 struct alifb_info * get_alifb_info()
 {
 	return m_fb_info;
 }
#else
...
#ifdef CONFIG_FB_UMP_ALI
int (*disp_get_ump_secure_id) (struct fb_info *info, struct alifb_info *g_fbi,
...
RPC_EXIT:			
    ...
    case GET_UMP_SECURE_ID:
        FB_PRF("%s : UMP cmd %x arg %x id %d\n", __FUNCTION__, (int)cmd, (int)arg, info->id);
		{
			if (!disp_get_ump_secure_id)
				request_module("ali_ump");
			if (disp_get_ump_secure_id) {
				ret = disp_get_ump_secure_id(fbinfo, info, arg,
							      secure_id_buf_num);
                FB_PRF("disp_get_ump_secure_id, ret=%d\n", ret);
            } else {
                FB_PRF("can't load ali_ump.ko\n");
				return -ENOTSUPP;
            }
		}
            break;
...
static int alifb_set_par(struct fb_info *fbinfo)
{
	...
	if(info->standard_gma_region_init)
    {
        ...
        if(var->bits_per_pixel == 8)
			dis_format = dis_format_to_ge(DIS_FORMAT_CLUT8);
		else if(var->bits_per_pixel == 16)
		{
			if(var->green.length == 6)
				dis_format = dis_format_to_ge(DIS_FORMAT_RGB565);
			else
				dis_format = dis_format_to_ge(DIS_FORMAT_ARGB1555);
		}
		else if(var->bits_per_pixel == 32)
		{
			if(var->transp.length == 8)
			    dis_format = dis_format_to_ge(DIS_FORMAT_ARGB8888);
			else if(var->transp.length == 0)
				dis_format = dis_format_to_ge(DIS_FORMAT_RGB888);
		}

		if((gma_info->bpp == (var->bits_per_pixel>>3))
			&& (gma_info->w == var->xres)
			&& (gma_info->h == var->yres)
			&& (gma_info->dis_format == dis_format))
		{
			 ...
			 u_int transp, struct fb_info *fbinfo)
{
	...
	if(regno > 256)
		return -EINVAL;
	color = ((red >> 0) & 0xf800) | ((green >> 5) & 0x07e0)|((blue  >> 11) & 0x001f);
	((uint32 *)(fbinfo->pseudo_palette))[regno] = color;
    FB_PRF("reg = %d, color = 0x%x\n", regno, color);
	return 0;
}
#else
static int alifb_setcolreg(u_int regno, u_int red, u_int green, u_int blue,
...
static int alifb_setcmap(struct fb_cmap *cmap, struct fb_info *fbinfo)
{
	...
	WARN_ON(info->palette_buffer==NULL);
	WARN_ON(red==NULL);
	WARN_ON(green==NULL);
	WARN_ON(blue==NULL);
	//WARN_ON(transp==NULL);
	...
	for (i = 0; i < cmap->len; i++) 
	{
        ...
        if(NULL != transp) {
            ...
            memcpy((void *)(info->palette_buffer+i*4+3), (void *)&deftrans, sizeof(unchar));
        }
        
        //copy to palette
        int _bpp = fbinfo->var.bits_per_pixel;
        if(_bpp == 32) {
            memcpy(fbinfo->pseudo_palette + i * 4, info->palette_buffer + i * 4, 4);
        } else if  (_bpp == 16) {
            unchar *pat = info->palette_buffer;
            unsigned char b = *(pat + i*4);
            unsigned char g = *(pat + i*4 + 1);
            unsigned char r = *(pat + i*4 + 2);
            int clr = (b >> 3) | ((g >> 2) << 5) | ((r >> 3) << 11);
            
            memcpy(fbinfo->pseudo_palette + i * 4, &clr, 4);
        }
...
struct fb_var_screeninfo g_alifb_default_var_argb565 = 
{
	720, 576, 720, 576, 0, 0,  /* virtual and visible resolution */
	16, 0, /* bits per pixel and grayscale flag */
	{11, 5, 0}, {5, 6, 0 }, {0, 5, 0 }, {0, }, /*bit field for RGBT */ 
	0, 0, -1, -1, 	
	0,	/* accel_flags */
}; 
EXPORT_SYMBOL(g_alifb_default_var_argb565);

struct fb_var_screeninfo g_alifb_default_var_rgb888 = 
{
	720, 576, 720, 576, 0, 0,  /* virtual and visible resolution */
	24, 0, /* bits per pixel and grayscale flag */
	{16, 8, 0}, {8, 8, 0 }, {0, 8, 0}, {0, }, /*bit field for RGBT */ 
	0, 0, -1, -1, 	
	0,	/* accel_flags */
}; 
EXPORT_SYMBOL(g_alifb_default_var_rgb888);
...
struct fb_var_screeninfo g_alifb_default_var_xrgb8888 = 
{
	720, 576, 720, 576, 0, 0,  /* virtual and visible resolution */
	32, 0, /* bits per pixel and grayscale flag */
	{16, 8, 0}, {8, 8, 0 }, {0, 8, 0}, {24, 0, 0 }, /*bit field for RGBT */
	0, 0, -1, -1, 	
	0,	/* accel_flags */
}; 
EXPORT_SYMBOL(g_alifb_default_var_xrgb8888);
...
static int set_fbinfo_in_default(struct fb_info *fbinfo, int id)
{
	...
	if(id == 3)
	{
		...
		if(g_fb2_bpp == 1)
			...
			memcpy((void *)&fbinfo->var, (void *)&g_alifb_default_var_argb565, sizeof(fbinfo->var));
        else  if(g_fb2_bpp == 3)
			memcpy((void *)&fbinfo->var, (void *)&g_alifb_default_var_rgb888, sizeof(fbinfo->var));    
		else{
			if(usr_ignore_alpha==0)
				memcpy((void *)&fbinfo->var, (void *)&g_alifb_default_var_argb8888, sizeof(fbinfo->var));	
			else
				memcpy((void *)&fbinfo->var, (void *)&g_alifb_default_var_xrgb8888, sizeof(fbinfo->var));	
		}
		#else
		if(g_fb2_bpp != 1)
		{
    ...
    memset(fbinfo->pseudo_palette, 0, 1024);
...
static int __init alifb_probe(struct platform_device *pdev)
{
	...
	if((g_support_standard_fb != 0) && (pdev->id == 1)){
		ret = init_stadard_gma_region(fbinfo);
		if(ret < 0){
			FB_PRF("fb%d : init standard gma region fail\n", fbinfo->node);
			goto EXIT;
		}
		info->standard_gma_region_init = 1;
	}
