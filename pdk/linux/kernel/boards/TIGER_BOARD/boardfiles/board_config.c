...
static void set_global_setting(void)
{
	...
	*/ 	
	...
	printk("arm_get_early_mem_size(): 0x%x\n",arm_get_early_mem_size());
...
void customize_board_setting(void)
{
	...
	printk("customize_board_setting set_global_setting()\n");
	set_global_setting();
	if(__G_ALI_BOARD_TYPE == 0x00004001)
	{
		printk("customize_board_setting board_setting_bga445()\n");
		board_setting_bga445();
	}
	else if(__G_ALI_BOARD_TYPE == 0x00004002)
	{
		printk("customize_board_setting board_setting_gfp256()\n");
...
#endif
	printk("customize_board_setting done\n");
}
