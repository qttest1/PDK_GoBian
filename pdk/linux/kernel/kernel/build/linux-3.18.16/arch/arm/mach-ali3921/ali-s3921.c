...
*/
    printk("L2x0 init before: CPU_REG_CTRL:0x%x, TAG RAM CTRL:0x%x, DATA RAM CTRL:0x%x\n", __REG32ALI(0x18000094), readl_relaxed(A9_L2_BASE_ADDR+L310_TAG_LATENCY_CTRL), readl_relaxed(A9_L2_BASE_ADDR+L310_DATA_LATENCY_CTRL));
    ...
    aux_ctrl_enable = L310_AUX_CTRL_EARLY_BRESP | /*(0x0 << L2X0_AUX_CTRL_ASSOCIATIVITY_SHIFT) |*/ (0x03 << L2C_AUX_CTRL_WAY_SIZE_SHIFT);
    aux_ctrl_enable |= (L310_AUX_CTRL_DATA_PREFETCH	| L310_AUX_CTRL_INSTR_PREFETCH | L310_AUX_CTRL_EARLY_BRESP);
    aux_ctrl_enable |= (0x01 << 23) | 1; //Non-write allocate
    aux_ctrl_enable |= L2C_AUX_CTRL_SHARED_OVERRIDE; //Mali Integration Guide Pg 2-11 enable bit 22      
...
//printk("L2x0 end: CPU_REG_CTRL:0x%x, L2X0_CTRL:0x%x, TAG RAM CTRL:0x%x, DATA RAM CTRL:0x%x\n", __REG32ALI(0x18000094),readl_relaxed(A9_L2_BASE_ADDR+L2X0_CTRL), readl_relaxed(A9_L2_BASE_ADDR+L310_TAG_LATENCY_CTRL), readl_relaxed(A9_L2_BASE_ADDR+L310_DATA_LATENCY_CTRL));
printk("L2x0 end: CPU_REG_CTRL:0x%x, L2X0_CTRL:0x%x, TAG RAM CTRL:0x%x, DATA RAM CTRL:0x%x, L2X0_PREFETCH_CTRL:0x%x\n", __REG32ALI(0x18000094),readl_relaxed(A9_L2_BASE_ADDR+L2X0_CTRL), readl_relaxed(A9_L2_BASE_ADDR+L310_TAG_LATENCY_CTRL), readl_relaxed(A9_L2_BASE_ADDR+L310_DATA_LATENCY_CTRL), readl_relaxed(A9_L2_BASE_ADDR+L310_PREFETCH_CTRL));
    data_rd = readl_relaxed(A9_L2_BASE_ADDR+L310_PREFETCH_CTRL);
    data_rd |= (1<<23) | (1<<24) | (1<<27) | (1<<28) | (1<<29) | (1<<30);
    data_rd &= (~0x1f);
    writel_relaxed(data_rd,A9_L2_BASE_ADDR+L310_PREFETCH_CTRL);
...
DT_MACHINE_START(ALI_S3921, BOARD_NAME)
	  //.atag_offset    = 0x100,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0))    
