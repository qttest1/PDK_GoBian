...
_mali_osk_errcode_t mali_mmu_reset(struct mali_mmu_core *mmu)
{
	...
	if (_MALI_OSK_ERR_OK == mali_mmu_raw_reset(mmu)) {
		...
		mali_hw_core_register_write(&mmu->hw_core, MALI_MMU_REGISTER_DTE_ADDR, mali_empty_page_directory_phys&0x7FFFFFFF); //20150319 Yashi:add 0x7FFFFFFF mask for CPU offset
...
static void mali_mmu_activate_address_space(struct mali_mmu_core *mmu, u32 page_directory)
{
	...
	mali_hw_core_register_write(&mmu->hw_core, MALI_MMU_REGISTER_DTE_ADDR, page_directory&0x7FFFFFFF); ////20150319 Yashi:add 0x7FFFFFFF mask for CPU offset
	mali_hw_core_register_write(&mmu->hw_core, MALI_MMU_REGISTER_COMMAND, MALI_MMU_COMMAND_ZAP_CACHE);
