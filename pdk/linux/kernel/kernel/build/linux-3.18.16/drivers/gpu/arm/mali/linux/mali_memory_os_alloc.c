...
void mali_mem_os_release_table_page(mali_dma_addr phys, void *virt)
{
	...
	if (MALI_MEM_OS_PAGE_TABLE_PAGE_POOL_SIZE > mali_mem_page_table_page_pool.count) {
		...
		dma_free_attrs(&mali_platform_device->dev,
			       _MALI_OSK_MALI_PAGE_SIZE, virt, phys|0x80000000,
			       &dma_attrs_wc); //20150319 Yashi add 0x80000000 OR, for CPU offset
#else
		dma_free_writecombine(&mali_platform_device->dev,
				      _MALI_OSK_MALI_PAGE_SIZE, virt, phys|0x80000000); //20150319 Yashi add 0x80000000 OR, for CPU offset
#endif
	}
}
...
static void mali_mem_os_page_table_pool_free(size_t nr_to_free)
{
	...
	for (i = 0; i < nr_to_free; i++) {
		...
		dma_free_attrs(&mali_platform_device->dev, _MALI_OSK_MALI_PAGE_SIZE,
			       virt_arr[i], (dma_addr_t)phys_arr[i]|0x80000000, &dma_attrs_wc); //20150319 Yashi add 0x80000000 OR, for CPU offset
#else
		dma_free_writecombine(&mali_platform_device->dev,
				      _MALI_OSK_MALI_PAGE_SIZE,
				      virt_arr[i], (dma_addr_t)phys_arr[i]|0x80000000); //20150319 Yashi add 0x80000000 OR, for CPU offset
#endif
	}
}
