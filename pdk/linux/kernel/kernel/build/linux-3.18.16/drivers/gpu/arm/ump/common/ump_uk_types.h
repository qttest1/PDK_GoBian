...
#ifdef ALI_UMP_MAPPING_KERNEL
/**
 * MAP_PHY ([out] u32 secure_id, [in,out] u32 size,  [in] phy_adr)
 */
typedef struct _ump_uk_map_phy_s
{
	void *ctx;                       /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;                   /**< Return value from DD to Userdriver */
	u32 ump_handle;                   /**< Return ump handle pointer from DD to Userdriver */
	u32 size;                        /**< Input and output. Requested size; input. Returned size; output */
	u32 phy_adr;  			 /**< Input dedecated physical address */
} _ump_uk_map_phy_s;
#endif
