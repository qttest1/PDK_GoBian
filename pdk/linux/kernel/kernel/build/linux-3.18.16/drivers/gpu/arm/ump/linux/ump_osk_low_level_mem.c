...
_mali_osk_errcode_t _ump_osk_mem_mapregion_map(ump_memory_allocation *descriptor, u32 offset, u32 *phys_addr, unsigned long size)
{
	...
	IO_phys_addr = (unsigned long)(*phys_addr) | 0x80000000; // Modify for fitting both os/decidated scenario, 20140514 by Tommy Chen
	...
	retval = remap_pfn_range( vma, ((u32)descriptor->mapping) + offset, IO_phys_addr >> PAGE_SHIFT, size, vma->vm_page_prot) ? _MALI_OSK_ERR_FAULT : _MALI_OSK_ERR_OK;;
		DBG_MSG(4, ("Mapping virtual to physical memory. ID: %u, vma: 0x%08lx, virtual addr:0x%08lx, CPU_phy_adr: 0x%08lx, IO_phy_adr: 0x%08lx, size:%lu, prot:0x%x, vm_flags:0x%x RETVAL: 0x%x\n",
		    ump_dd_secure_id_get(descriptor->handle),
		    (unsigned long)vma,
		    (unsigned long)(vma->vm_start + offset),
		        IO_phys_addr,
		        IO_phys_addr - 0x80000000,		        
		    size,
