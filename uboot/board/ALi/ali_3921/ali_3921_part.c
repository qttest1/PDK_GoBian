...
int load_part_ext(const char *partName, u_char *LoadAddr, loff_t offset, size_t len)
{
    ...
    ret = load_part_nand_ext(partName, LoadAddr, offset, len);
        ...
        ret = load_part_sd_ext(partName, LoadAddr, offset, len);
    #else
...
int load_part_sector(const char *partName, u_char *LoadAddr, int bufLen)
{
	...
	return ret;
#elif defined(CONFIG_ALI_SD)//for gobian 
    u_char *buf = NULL;
    ...
    if(buf == NULL)
     {
      ...
      readsize += ali_read_file_at(partName, 12, buf, size);
      memcpy(LoadAddr, buf, bufLen);      
...
int load_part_ubo(const char *partName, u_char * LoadAddr)
{
	...
	printf("<%s>(%d): %s read addr: 0x%08x\n", __FUNCTION__, __LINE__, partName, LoadAddr);
        ...
        int len = ali_get_filelength(partName);

        if (load_part_ext(partName, LoadAddr, 0, len)<0)
	{
		printf("<%s>(%d): %s read data error\n", __FUNCTION__, __LINE__, partName);
		return -3;
	}

        hdr = (image_header_t*)malloc(sizeof(image_header_t));

        memcpy(hdr, LoadAddr, sizeof(image_header_t));
...
#if defined(CONFIG_ALI_SD)
	if (ali_read_file_at(partName, 0, preReadBuffer, preReadSize) < 0)
#else
...
#if defined(CONFIG_ALI_SD)
int load_part_ubo_comp_with_sig_ext(const char *partName, const int idx, u_char *LoadAddr, u_char *sig)
{
	size_t preReadSize;
	image_header_t *hdr = NULL;
	uint32_t *size;
    ...
    len = ali_get_filelength(partName);

    buf = (unsigned char*)malloc(len);

    if (buf == NULL)
	{
		printf("<%s>(%d): %s malloc buffer fail\n", __FUNCTION__, __LINE__, partName);
		//return -1;
	}

        printf("<%s>(%d): %s file len: %d\n", __FUNCTION__, __LINE__, partName, len);
       int ret = ali_read_file_at(partName, 0, LoadAddr, len);
	...
	if (idx < count)
	{
        ...
        memcpy(LoadAddr, buf+offset, len);
...
#if defined(CONFIG_ALI_SD)
	if (ali_read_file_at(partName, 0, LoadAddr, preReadSize) < 0)
#else
	if (load_part_ext(partName, LoadAddr, 0, preReadSize) < 0)
#endif
	{
...
#if defined(CONFIG_ALI_SD)
		if (ali_read_file_at(partName, (int)offset, LoadAddr, (int)len) < 0)
#else
		if (load_part_ext(partName, LoadAddr, offset, len) < 0)
#endif
		{
...
int part_ubo_check(UINT8 *entry)
{
	...
	if (be32_to_cpu(hdr->ih_magic) != IH_MAGIC)
	{
		printf("<%s>(%d): Bad Magic Number, magic: 0x%08x\n", __FUNCTION__, __LINE__, be32_to_cpu(hdr->ih_magic));
		return -1;
	}
...
int load_bootmedia(void)
{
    ...
    printf("load bootmedia: entry: 0x%08x\n", entry);
	...
	//printf("<%s>(%d): %s read addr: 0x%08x\n", __FUNCTION__, __LINE__, partName, LoadAddr);
        ...
        int len = ali_get_filelength(fileName);

	printf("<%s>(%d): %s file len: %d\n", __FUNCTION__, __LINE__, fileName, len);

        ali_read_file_at(fileName, 0, entry, len);

        hdr = (image_header_t*)malloc(sizeof(image_header_t));

        memcpy(hdr, entry, sizeof(image_header_t));
...
int load_main_bin(void)
{
	...
	ADF_BOOT_BOARD_INFO *info = (ADF_BOOT_BOARD_INFO *)PRE_DEFINED_ADF_BOOT_START;
       ...
       printf("load main_bin");

       /* Load see/ae first to show boot logo/media earlier */
       strcpy(fileName, PART_NAME_AUD_CODE);
	entry = (u_char *)(info->memmap_info.ae_start | 0x80000000) - UBO_HEAD_LEN;
	printf("Loading comp '%s' to 0x%x...\n", fileName, entry);
	len = load_part_ubo(fileName, entry);
	if (len < 0)
	{
		printf("Load comp '%s' Fail\n", fileName);
		return -1;
	}
	if (part_check(fileName, entry, len, sig)!= RET_SUCCESS)
	{
		return -1;
	}

       strcpy(fileName, PART_NAME_SEE);
	entry = (u_char *)(info->memmap_info.see_start | 0x80000000) - UBO_HEAD_LEN;
	printf("Loading comp '%s' to 0x%x...\n", fileName, entry);
	len = load_part_ubo(fileName, entry);
	if (len < 0)
	...
	/* Load kernel */
       strcpy(fileName, PART_NAME_KERNEL);
	entry = (u_char *)(info->memmap_info.kernel_start | 0x80000000) - UBO_HEAD_LEN;
	printf("Loading comp '%s' to 0x%x...\n", fileName, entry);
	len = load_part_ubo(fileName, entry);
	if (len < 0)
	...
	char bootm_cmd_str[64];
	memset(bootm_cmd_str, 0, sizeof(bootm_cmd_str));
	sprintf(bootm_cmd_str, "bootm 0x%08x", entry);
	setenv("bootcmd", bootm_cmd_str);
