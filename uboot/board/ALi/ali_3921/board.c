...
# endif /* CONFIG_SYS_FLASH_CHECKSUM */
    ...
    udelay(3000);
    printf("start to check sdcard\n");

    int usb_sd_flag=0;
    unsigned int g_timer_ticks=0;
    struct mmc *mmc;
    int ret=0;
    static int g_curr_device = -1;
    ...
    if (g_curr_device < 0) {
     ...
     printf("start to find mmc devices\n");
    mmc = find_mmc_device(g_curr_device);
    if (!mmc) {
     ...
     printf("start to mmc init\n");
    mmc->has_init = 0;
    ret = mmc_init(mmc);	
    if(ret!=0){
    	printf("mmc_init(), ret %d\n",ret);	
    	if(ret==NO_CARD_ERR){
    	    printf("No SD Card In Slot,System will try to USB device\n");
    	}
    }
    else{
        usb_sd_flag=1;
...
FAT_REGISTER:
    ...
    printf("start to fat_register\n");
    g_timer_ticks = get_timer(0);
    ret=0;
    if(usb_sd_flag==0){
    	printf("usb and sd not scaned**\n");
    	ret =1;
    	goto EXIT;
    }	

    //registe fat system
    printf("ali_dev_fat_register , usb_sd_flag[%d] \n", usb_sd_flag);
    if(0!=ali_dev_fat_register(0,usb_sd_flag)){	
        ret =1;
        printf("register fat system fail**\n");
        goto EXIT;
    }

    printf("ali_set_config\n");
