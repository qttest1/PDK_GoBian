...
/*
static void parse_gw_config()
{
	char cmd[1024*1024];
       memset(cmd, 0, 1024*1024);
	ali_read_file("config.txt", cmd);
...
/*extern int ali_parse_configtxt(char *inifile,struct GW_CONFIGS * config);
void set_avinfo_from_gwconfig(ADF_BOOT_AVINFO *avinfo, struct GW_CONFIGS config)
{
	if(config.tvSystem!=0xff)
		avinfo->tvSystem=config.tvSystem;
	if(config.osd_ratio!=0xff)
		avinfo->osd_ratio=config.osd_ratio;
	printf("set_avinfo_from_gwconfig  tvSystem=%d  osd_ratio=%d \n",avinfo->tvSystem,avinfo->osd_ratio);
}*/
int set_boardinfo(int isRecovery)
{
	...
	/*struct GW_CONFIGS gwconfig;
	gwconfig.tvSystem=0xff;
	gwconfig.osd_ratio=0xff;
	memset(gwconfig.macStr,0,sizeof(gwconfig.macStr));*/
	boardinfo->avinfo.framebuffer_ignore_alpha=1;//as default
	if(ali_parse_configtxt("config.txt",boardinfo)>=0)
	{
		//set_avinfo_from_gwconfig(&boardinfo->avinfo,gwconfig);
		//memcpy((u_char*)&boardinfo->macinfo, gwconfig.macStr, sizeof(gwconfig.macStr));
		printf("ali_parse_configtxt   OK\n");
	}
