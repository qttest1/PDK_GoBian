 ...
 * Return the number of bytes read or -1 on fatal errors.
 */
//__u8 get_contents_vfatname_block[MAX_CLUSTSIZE] __aligned(32);
__u8 get_contents_vfatname_block[MAX_CLUSTSIZE];

static long
get_contents_at (fsdata *mydata, dir_entry *dentptr, unsigned long pos,__u8 *buffer,
	      unsigned long maxsize)
{
	unsigned long filesize = FAT2CPU32(dentptr->size), gotsize = 0;
	unsigned int bytesperclust = mydata->clust_size * mydata->sect_size;
...
long
do_fat_read_filelength (const char *filename, void *buffer, unsigned long maxsize,
	     int dols)
{
	char fnamecopy[2048];
	boot_sector bs;
...
long
do_fat_read_at (const char *filename, unsigned long pos,void *buffer, unsigned long maxsize,
	     int dols)
{
	char fnamecopy[2048];
	boot_sector bs;
...
rootdir_done:
	...
	ret = get_contents_at(mydata, dentptr, pos,buffer, maxsize);
	//unsigned long filesize = FAT2CPU32(dentptr->size);
	//ret = FAT2CPU32(dentptr->size);
	debug("Size: %d, got: %ld\n", FAT2CPU32(dentptr->size), ret);
...
int file_fat_read_filelength (const char *filename, void *buffer, unsigned long maxsize)
{
	//printf("get filelength %s\n", filename);
	return do_fat_read_filelength(filename, buffer, maxsize, LS_NO);
}
